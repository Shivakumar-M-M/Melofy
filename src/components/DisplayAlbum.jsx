import React from 'react'
import Navbar from './Navbar'   
import { useParams } from 'react-router-dom'
import assets, { albumsData, songsData } from '../assets/assets';

const DisplayAlbum = () => {
    const { id } = useParams();
    
    const albumbData = albumsData[id];
    
    
  return (
    <>
      <Navbar></Navbar>
      <div className='flex gap-8  flex-col mt-10 md:flex-row md:items-end'>
        <img className=" w-48 rounded" src={albumbData.image}></img>
        <div className='flex flex-col'>
            <p>PlayList</p>
            <h2 className='text-5xl font-bold mb-4 md:text-7xl'>{albumbData.name}</h2>
            <h4>{albumbData.desc}</h4>
            <p className='mt-1'>
                <img className='w-10 inline-block' src={assets.spotify_logo} alt=""></img>
                <b>Melofy </b>
                 * 13,23,234 Likes
                 * <b>50 songs</b>
                 about 2hr 30 min
            </p>
        </div>
      </div>
      <div className='grid grid-cols-3 sm:grid-cols-4 mt-10 mb-4 pl-2 text-[#a7a7a7]'>
        <p className='mr-4 '>#<b>Title</b></p>
        <p>Albumb</p>
        <p className='hidden sm:block'>Date added</p>
        <img className="m-auto w-4"src={assets.clock_icon}></img>
      </div>
      <hr></hr>
      {
        songsData.map((item,index)=>(
            <div key={index} className='grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer'>
                <p className='text-white'>
                    <b className='mr-4 text-[#a7a7a7]'>{index+1}</b>
                    <img className="inline w-10 mr-5"src={item.image}></img>
                    {item.name}
                </p>
                <p className='text-[15px]'>{albumbData.name}</p>
                <p className='text-[15px] hidden sm:block'>5 Days ago</p>
                <p className='text-[15px text-center '>{item.duration}</p>
            </div>
        ))
      }
    </>
  )
}

export default DisplayAlbum
